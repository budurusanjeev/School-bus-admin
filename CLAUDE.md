...
The source file is capitalized `Index.html`. Render's Linux static host looks for lowercase `index.html` at `/`, so `render.yaml` copies `Index.html` → `index.html` at build time. Do not gitignore `index.html` on Windows: Git is case-insensitive here and will ignore `Index.html` too.
...
- **Auth is server-verified now**: the login gate (`#auth-gate`, `checkAuth()`) sends the entered password to `GET /api/v1/admin/auth-check` in the `x-admin-key` header; the backend compares it to its `ADMIN_PASSWORD` env var. On success the key is kept in `sessionStorage` (auto-unlock on reload of the same tab) and attached to **every** admin request via `adminHeaders()` — `submitJson()`, `tryFetchList()`, and the fleet-locations poll all include it. `submitJson()`, `tryFetchList()`, and the fleet poll treat HTTP 401 as "key revoked" (`handleUnauthorized()` clears the key and reloads to the gate). `verifyKey()` treats 401 as a wrong password and any other non-OK status as a server/network failure (so a 500 does not look like "Incorrect password"). No password lives in this file. When adding any new admin fetch, always spread in `adminHeaders()` and handle 401 the same way.
- **Layout gotcha**: `body` is `display: flex` with explicit `width: 100vw`/`height: 100vh`. `#app-root` (which wraps `#sidebar` + `#map-wrapper`, toggled from `display:none` to `display:flex` by `unlock()` once auth succeeds) is therefore itself a flex *item* of `body` and needs its own explicit `width: 100%; height: 100%` — without it, it shrinks to fit just the 380px sidebar and the map gets zero width. If you ever restructure the top-level layout, keep this in mind.
...
- **Live fleet tracking**: `trackLiveFleetPipeline()` polls `GET /admin/fleet/locations` every 5 seconds (`setInterval`, started from `startApp()` after auth succeeds — not before) and reconciles the result against `activeLiveBusMarkers` (an in-memory map of `bus_id -> Leaflet marker`) — updating positions for buses still reporting, adding new markers for newly-seen buses, and removing markers for buses that stopped reporting. The backend currently returns **every** bus and COALESCE-fills missing GPS with the Mahbubnagar hub; the panel treats a missing `timestamp` as "not reporting" so parked/untracked buses do not all pile up on the hub. The `#live-fleet-counter` badge counts only those reporting buses. The backend also has `GET /admin/fleet/routes-with-stops`; this panel does not draw stop polylines yet.
...
- Production URL: `https://school-bus-admin.onrender.com`
- GitHub: `https://github.com/budurusanjeev/School-bus-admin` (branch `main`)
- This site still calls `https://school-bus-backend-2.onrender.com`. The backend currently allows all origins (and `x-admin-key` on preflight). If that is later restricted, add `https://school-bus-admin.onrender.com` to the backend CORS allowlist.
